...
namespace DigitalRuby.WeatherMaker
{
    ...
    public static class WeatherMakerEditorCommands
    {
        ...
        private const string urpAutoDetectShownKey = "WeatherMaker_URPAutoDetectPopupShown";
        ...
        private static bool IsURPPackageVersion17OrNewer()
        {
            ...
            if (dotIndex > 0 && int.TryParse(version.Substring(0, dotIndex), out int major))
            {
                return major >= 17;
            }
            return false;
        ...
        /// <summary>
        /// Checks if URP 17+ is installed but UNITY_URP is not defined, and shows a one-time popup
        /// offering to enable URP support for Weather Maker. Called from AutoConfigureRenderPipeline.
        /// </summary>
        public static void TryPromptAutoEnableURP()
        {

#if !UNITY_URP

            if (EditorPrefs.GetBool(urpAutoDetectShownKey, false))
            {
                return;
            }

            if (!IsURPPackageVersion17OrNewer())
            {
                return;
            }

            // Mark as shown immediately to prevent duplicate popups when DidReloadScripts
            // is invoked multiple times during a single editor initialization cycle
            EditorPrefs.SetBool(urpAutoDetectShownKey, true);

            // Defer to next editor frame to avoid issues during initialization
            EditorApplication.delayCall += () =>
            {
                if (EditorUtility.DisplayDialog("Weather Maker - URP Detected",
                    "Universal Render Pipeline has been detected in this project but Weather Maker is not configured for it.\n\n" +
                    "Would you like to enable URP support now?\n\n" +
                    "(This can also be done later via Window > Weather Maker > Enable Universal Render Pipeline)",
                    "Yes, Enable URP", "No Thanks"))
                {
                    EnableURP();
                }
            };
