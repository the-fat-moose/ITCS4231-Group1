...
fixed4 temporal_reprojection_fragment_custom(WEATHER_MAKER_TEMPORAL_REPROJECTION_FRAGMENT_TYPE i) : SV_TARGET
{
	...
	// Stereo instancing path: keep temporal accumulation active, but avoid the
	// matrix reprojection branch that some Metal/D3D compiler paths miscompile.
	UNITY_BRANCH
	if (weatherMakerTemporalReprojectionEnabled == 2)
	{
		...
		fixed4 prev = WM_SAMPLE_FULL_SCREEN_TEXTURE_SAMPLER(_TemporalReprojection_PrevFrame, _point_clamp_sampler, i.uv.xy);
		...
		if (prev.a > 0.0)
		{
			...
			fixed4 diff = fixed4Zero;
			diff.y = Luminance(cur.rgb);
			diff.z = Luminance(prev.rgb);
			diff.x = abs(diff.y - diff.z);
			diff.w = abs(cur.a - prev.a);
			cur = WEATHER_MAKER_TEMPORAL_REPROJECTION_BLEND_FUNC(prev, cur, diff, float4(i.uv.xy, i.uv.xy), i);
			UNITY_BRANCH
			if (cur.a > 0.0)
			{
				return cur;
	...
	if (weatherMakerTemporalReprojectionEnabled == 2)
	{
		...
		else
		{
			...
			previousUV.x -= (0.5 * eyeIndex);
			previousUV.x *= 2.0;

#endif

			float4 reproj = float4(0.0, 0.0, -1.0, 1.0);
			...
			else
			{
				...
				// Avoid dynamic matrix array indexing in fragment shader; Metal/D3D compiler
				// can emit invalid bytecode for some stereo-instancing variants.
				UNITY_BRANCH
				if (eyeIndex < 0.5)
				{
					reproj = mul(_TemporalReprojection_ipivpvp[0], prevPos);
				}
				else
				{
					reproj = mul(_TemporalReprojection_ipivpvp[1], prevPos);
				}
