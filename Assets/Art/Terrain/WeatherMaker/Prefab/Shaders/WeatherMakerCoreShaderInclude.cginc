...
float GetFullScreenRayLength(float3 forwardLine)
{
	if (WM_CAMERA_RENDER_MODE_CUBEMAP)
	{
		// cubemap forwardLine is NDC coords, not view-space direction
		// compute the view-space ray length via inverse projection to match normal camera behavior

#if defined(UNITY_REVERSED_Z)

		float4 clipPos = float4(forwardLine.xy, GetNearPlane(), 1.0);
		...
		clipPos = mul(_WeatherMakerInverseProj[0], clipPos);
		clipPos.xyz /= clipPos.w;
		// inverse view is rotation only, preserves length
		return length(clipPos.xyz);
	}
	else
	{
		return length(forwardLine);
	}
}
