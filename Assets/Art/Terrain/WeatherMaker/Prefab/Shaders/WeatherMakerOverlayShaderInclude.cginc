...
// URP accurate GBuffer normal decode copied from URP Packing.hlsl.
float2 WMUnpack888UIntToFloat2(uint3 x)
{
...
float2 WMUnpack888ToFloat2(float3 x)
{
	uint3 i = (uint3)(x * 255.5); // +0.5 to fix precision error on iOS
	return WMUnpack888UIntToFloat2(i);
}

float3 WMUnpackNormalOctQuadEncode(float2 f)
{
...
bool GetOverlayInputs(float2 screenUV, float3 forwardLine, out float3 rayDir, out float3 worldPos, out float3 normal, out float depth)
{
	...
	if (_WeatherMakerDeferredShading)
	{
		...
		// no geometry at this pixel, avoid undefined normal decode.
		clip(-0.5 + (depth < 0.99999));

		// normal is already world space

#if UNITY_URP

		float3 cameraNormalTex = WM_SAMPLE_FULL_SCREEN_TEXTURE(_CameraNormalsTexture, screenUV.xy).xyz;

		#if defined(_GBUFFER_NORMALS_OCT)
		
		// accurate gbuffer mode packs oct normal into 888.
		float2 octNormalWS = (WMUnpack888ToFloat2(cameraNormalTex) * 2.0) - 1.0;
		normal = WMUnpackNormalOctQuadEncode(octNormalWS);
		
		#else

		// non-accurate mode stores signed world-space normal directly.
		normal = cameraNormalTex;
